...
#include "basics.h"
#include "lib_main.h"
#include "messages.h"        /* for NC_ABORT_F */
...
showfl()
{
  ...
  while (free != TERMINATOR) {
    nprintf(OF_DEBUG_LOG, " %d (%d)", free, blksize(free));
    free = fwdlink(free);
  }
...
chkfl()
{
  ...
  while (free != TERMINATOR) {
    if (freetag(free) != FREETAG || endptr(free) != -free) {
      nprintf(OF_DEBUG_LOG, "block %d on free chain not tagged properly\n", free);
      nabort(NC_ABORT_F);
    }
    if (fwdlink(bcklink(free)) != free) {
      nprintf(OF_DEBUG_LOG, "block %d on free chain not linked properly\n", free);
      nabort(NC_ABORT_F);
    }
    free = fwdlink(free);
  }
...
memchk()
{
  ...
  free = fwdlink(freelisthdr);
  if (free == TERMINATOR) {
    strcpy(buf, "the freelist is empty");
...
*/
  ...
  while (free != TERMINATOR) {
    if (freetag(free) != FREETAG || endptr(free) != -free) {
      strcpy(buf, "block on free chain not tagged properly");
      wb(addr);
      goto halt;
    }
    if (fwdlink(bcklink(free)) != free) {
      strcpy(buf, "block on free chain not linked properly");
      wb(addr);
      goto halt;
    }
    free = fwdlink(free);
...
*/
    if (!allocated(addr)) {  /* check that it ison the free list */
      free = fwdlink(freelisthdr);
      while (addr != free && free != TERMINATOR)
        free = fwdlink(free);
      if (free != addr) {
        sprintf(buf, "free block encountered not on chain %d\n", addr);
        puts(buf);
        showfl();
        goto halt;
      }
      ...
      if (endptr(addr) < 0) {
        nprintf(OF_DEBUG_LOG, "endptr marker wrong on allocated block %d\n", addr);
        goto halt;
      }
      if (blksize(addr) < minsize) {
        sprintf(buf, "small allocated block %d of size %d \n", addr, blksize(addr));
        wb(addr);
        goto halt;
      }
    }
  ...
  if (addr != memsize) {
    sprintf(buf, "addr in memchk does not match memsize");
    goto halt;
  }
  return "success";
halt:
