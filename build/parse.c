...
static int  DEFN(nialptr * tree, int inblock);
static int  ACTION(nialptr * tree);
static int  EXPRESSION(nialptr * tree);
static int  PRIMARY(nialptr * tree);
static int  EXPR_SEQ(nialptr * tree);
static int  STMT_EXPR(nialptr * tree);
static int  COMMENT(nialptr * tree);
static void IDLIST(nialptr * tree, int searchtype);
static int  ID(nialptr * tree, int searchtype);
...
static int  Xs_(void);
static int  Xao(void);
static int  XaRE(void);
static int  Xoa(void);
static int  Xoo(void);
static int  XoRE(void);
static int  XocRE(void);
static int  Xocp(void);
static int  Xoco(void);
static int  Xoct(void);
static int  Xto(void);
static int  Xtco(void);
#ifdef TR_EXPRESSIONS
static  int Xat(void );
static  int Xot(void );
static  int Xta(void );
static  int Xtca(void );
static  int XtcRE(void );
static  int Xttc(void );
static  int Xt_(void );
#endif
static int  OPERATION(nialptr * tree);
static int  TR_FORM(nialptr * tree);
static int  Xcmb(int t, nialptr(*b) (nialptr t1, nialptr t2));
static int  XcmbR(int t, nialptr(*b) (nialptr t1, nialptr t2));
static int  XxRE(int newform);
static int  XLERE(void);
static int  Xn_(void);
static int  X_n(void);
static int  XnRE(void);
static int  cant(void);
static int  quotepgm(void);
static int  rec_form(int exprflag);
static int  brackets(void);
static nialptr deref_sys(nialptr tree);
static int  shift(void);
static int  formfinder(nialptr * tree, int *k);
static int  OP_FORM(nialptr * tree);
static int  DEFN_SEQ(nialptr * tree, int inblock);
static int  BLOCK(nialptr * tree);
static int  BLOCKBODY(nialptr sym, nialptr args, nialptr * block);


static int  IX_VAR(nialptr * tree);
static int  SCOPED_VAR(nialptr * tree);
static int  IX_ASSIGN_EXPR(nialptr * tree);
static int  ASSIGN_EXPR(nialptr * tree);
static int  WHILE_EXPR(nialptr * tree);
static int  EXIT_EXPR(nialptr * tree);
static int  REPEAT_EXPR(nialptr * tree);
static int  FOR_EXPR(nialptr * tree);
static int  IF_EXPR(nialptr * tree);
static void ite_cleanup(int cnt);
static int  CASE_EXPR(nialptr * tree);
...
{ /* store the state checking that the state stack is not full */
  ...
  if (topstate == STATELIMIT) {
    topstate = -1;
    exit_cover(NC_PARSE_STACK_OVERFLOW_W);
  }
...
#ifdef TR_EXPRESSIONS
static int (*reducetab[100])() = {
/*         ---p--- ---s--- ---a--- ---o--- --oc--- ---t--- --tc---
              ---le-- ---re-- ---n---  */

/* P  */   Xpp,    cant,   cant,   Xp_,    cant,   Xp_,    Xp_,
              shift,  Xp_,    X_n,
/* S  */   Xsp,    cant,   cant,   Xs_,    cant,   Xs_,    Xs_,
              shift,  Xs_,    X_n,
/* A  */   cant,   cant,   cant,   Xao,    cant,   shift,  Xat,
              shift,  XaRE,   X_n,
/* O  */   shift,  shift,  Xoa,    Xoo,    cant,   shift,  Xot,
              shift,  XoRE,   X_n,
/* OC */   Xocp,   cant,   cant,   Xoco,   cant,   Xoct,   cant,
              shift,  XocRE,  X_n,
/* T  */   shift,  shift,  Xta,    Xto,    cant,   shift,  Xttc,
              shift,  Xt_,    X_n,
/* TC */   cant,   cant,   Xtca,   Xtco,   cant,   cant,   cant,
              shift,  XtcRE,  X_n,
...
/*         ---p--- ---s--- ---a--- ---o--- --oc--- ---t--- --tc---
  ...
  shift, Xp_, X_n,
   /* S  */ Xsp, cant, cant, Xs_, cant, Xs_, Xs_,
  shift, Xs_, X_n,
   /* A  */ cant, cant, cant, Xao, cant, shift, shift,
  shift, XaRE, X_n,
