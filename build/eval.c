...
/* Q'Nial header files */
#include "coreif_p.h"        /* for checksignal  and error messages */
...
#include "messages.h"        /* error codes */
...
{
  if (primroutines) {
/* NOTE...NOTE...BEWARE....BEWARE....
   The Borland 4.5 Compiler has a assembler level error (seems to
   to get confused with where its registers are) with the following
   version of the code:
   char       *tmp_str = slower(pfirstchar(bnames[get_index(p)]));

  ((*applytab[get_index(p)]) ());

  It seemed to be failing to keep the common use of get_index(p)
  around properly, and was clobbering it just before the function
  application
 */
    int         ind = get_index(p);
...
#  if (__BORLANDC__ == 0x460)
    ind = get_index(p);
#  else
//#    error Confirm this bug with different compiler versions
#  endif
#endif
    ((*applytab[ind]) ());
...
/*  routine that implements the primitive expression Toplevel that does
    a jump to the top level of the evaluator (out of CommandInterpret) */

void
itoplevel()
{
  exit_cover(NC_PROGRAM_BREAK_B);
}
...
coerceop()
{
  ...
  while (tag(op) == t_variable) {
    if (sym_trflg(get_entry(op))) /* do not coerce past an operation to be traced. 
                                     MAJ: Does this change the semantics? */
      break;
    op = fetch_var(get_sym(op), get_entry(op));
  }
...
/* routine to implement the primitive operation getname which converts a 
   ...
   MAJ: Ideally this function should be testing that the argument is legitimate.
   However this would require every symtab entry to be tagged. The code should
   at least confirm that the result is a string and fault otherwise.
*/
...
#ifdef DEBUG
          if (topstack != oldtop) {
            nprintf(OF_DEBUG, "system error: stack not empty in break loop\n");
...
dump_call_stack()
{
  ...
  for (cnt = 0; cnt < call_stack_pointer; cnt++) {
...
process_faultbreak(char *f)
{
  ...
  if (call_stack_pointer != 0 && !instartup) {
    ...
    triggered = true;
    longjmp(error_env, NC_FAULT_W); /* ws_cleanup called in coreif.c */
  }
}
