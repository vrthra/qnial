...
/* place any switch that removes all code in a file here:
   examples are DLL, PROFILE etc.
   */
...
static int  maxints(nialint * ptrx, nialint n);
...
static int  minints(nialint * ptrx, nialint n);
    ...
    case atype:
        if (tx == 0) {       /* create Nadir directly to avoid a trigger */
          apush(createatom(faulttype, "?O"));
               ...
               /* MAJ: use binary case for consistency */
      ...
      if (v != valence(y))
        ...
        if (z) {             /* shapes are equal */
          ...
          switch (kx) {
            case atype:
                {
                  nialptr    *ptrx = pfirstitem(x), /* safe in equal */
                             *ptry = pfirstitem(y); /* safe in equal */
