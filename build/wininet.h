...
//
  ...
  typedef struct {
    ...
    LPTSTR      lpszExtraInfo;  // pointer to extra information (e.g. ?foo or
                                // #foo)
...
//
  ...
  typedef struct _INTERNET_CACHE_ENTRY_INFOA {
    DWORD       dwStructSize;// version of cache system. ?? do we need this
                             // for all entries?
    LPSTR       lpszSourceUrlName;  // embedded pointer to the URL name
                                    // string.
    LPSTR       lpszLocalFileName;  // embedded pointer to the local file
                                    // name.
    ...
    FILETIME    LastModifiedTime; // last modified time of the file in GMT
                                  // format.
    ...
    LPSTR       lpszFileExtension;  // File extension used to retrive the
                                    // urldata as a file.
  ...
  typedef struct _INTERNET_CACHE_ENTRY_INFOW {
    ...
    LPSTR       lpszSourceUrlName;  // embedded pointer to the URL name
                                    // string.
    LPWSTR      lpszLocalFileName;  // embedded pointer to the local file
                                    // name.
    DWORD       CacheEntryType; // cache type bit mask.
    DWORD       dwUseCount;  // current users count of the cache entry.
    DWORD       dwHitRate;   // num of times the cache entry was retrieved.
    ...
    LPBYTE      lpHeaderInfo;// embedded pointer to the header info.
    DWORD       dwHeaderInfoSize; // size of the above header.
    LPWSTR      lpszFileExtension;  // File extension used to retrive the
                                    // urldata as a file.
    DWORD       dwReserved;  // reserved for future use.
