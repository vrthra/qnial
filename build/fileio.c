...
#include "coreif_p.h"        /* for IOContext information */
...
#if defined (WINDOWS_SYS)  || defined(UNIXSYS)
...
nprintf(int flags, char *fmt,...)
{
  ...
  if (rc >= 4999)
    exit_cover(NC_NPRINTF_BUFFER_OVERFLOW_I);
  /* no other flags set */
  if (flags & OF_NORMAL)
...
#else
#error nprintf not defined for this system
#endif
...
iopen()
{
  ...
  if (portno == nextfilenum) {  /* the file is not currently open. Set
    ...
    /* here we allow the assumed text mode switches */
      ...
      strcpy(mode, "pwt");
    /* two of the following four mode are there for the Windows version and
     * the "b" or "t" character will be clear in we are running a UNIXSYS
     * system (see below) */
    ...
    if (mode[0] == 'd') {
      ...
      fptr = _popen(gcharbuf, mode + 1);
      nprintf(OF_DEBUG, "_popen with \"%s\" \"%s\"\n", gcharbuf, mode + 1);
      nprintf(OF_DEBUG, "The error is: %s\n", strerror(errno));
...
#else
#error You must define a popen call for this machine/OS
#endif
      if (!fptr) {
  ...
  while (cont) {             /* allow for multiple reads to get a line */
    ...
    *ptrCbuffer = '\0';
    if (cnt > 0 && mode == 2)
       ...
       * expression when in mode 2 (from loaddefs) */
    {
...
writerec(int isarray, nialint portno, nialint index, nialptr record)
{
  ...
  if (isarray) {
    ...
    if (blocklen == IOERR) {
      exit_cover(NC_DIRECT_ACCESS_WRITE_W);
    }
  }
  else {
    long    cnt;
...
block_array(FILE * fpr, nialptr x)
{
  ...
  /* write the valence */
  v = valence(x);
  testerr(writeblock(fpr, (char *) &v, sizeof(int), true, nextrecpos, 0));
  nextrecpos += sizeof(int);
...
void
readinput()
{                /* Calls the read string routine provided in the
                    core interface to read a string and then converts it to
                    a Nial string. */
  char       *str;
  ...
  if (NC_readstr(&str)) {
    ...
    buildfault("read_string not installed");
}
...
ireadchar(void)
{
  ...
  if (NC_readchar(&ch)) {
    ...
    buildfault("readchar not installed");
  }
  checksignal(NC_CS_INPUT);
}
...
writelog(char *text, int textl, int newlineflg)
{
  ...
  if (fptr == OPENFAILED) {
    exit_cover(NC_LOG_OPEN_FAILED_W);
    keeplog = false;
  }
  /* write to logfile */
