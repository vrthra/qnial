/* ==============================================================
  ...
  This module is responsible for the display of Nial arrays for
...
static char stdformat[FORMAT_LEN],
            svdformat[FORMAT_LEN];
...
disp(nialptr x, int displaymode)
{
  ...
  switch (kind(x)) {
    ...
    case inttype:
        {
          if ((double) tx * NDINT + 1 > LARGEINT)
            exit_cover(NC_INTEGER_OVERFLOW_PICTURE_W);
          for (i = 0; i < tx; i++) {
            char        buf[NDINT + 1];
            nialint     intlen;

            sprintf(buf, "%d", fetch_int(x, i));
    ...
    case realtype:
        {
          if ((double) tx * NDREAL + 1 > LARGEINT)
            exit_cover(NC_INTEGER_OVERFLOW_PICTURE_W);
          for (i = 0; i < tx; i++) {
            char        buf[NDREAL + 1];
            int         reallen;
